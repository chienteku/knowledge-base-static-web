...
### Exercise 1: Shard Key Candidate Evaluation

**Problem:** You are sharding a `users` collection containing `10,000,000` documents. 
Evaluate these three candidate fields, and state which is the **Best** choice, and why:
-   `gender` (String: "M", "F", "Other")
-   `signup_timestamp` (Date: auto-generated at registration)
-   `user_id` (String: unique random UUID generated by the app)

**Expected output:**
```text
- `gender` is a POOR choice: Low cardinality. It will restrict data distribution to 3 shards maximum.
- `signup_timestamp` is a POOR choice: Monotonically increasing. Creates a write hotspot where all new registrations hit the same shard.
- `user_id` is the BEST choice: High cardinality (unique values) and random distribution (UUID). It ensures even write distribution across all shards and prevents bottlenecks.
```

> [!check]- Answer
> - Analyze the cardinality of gender.
> - Consider the sequential nature of timestamps during high registration traffic.

---



### Exercise 2: Shard Key Selection Criteria

**Problem:** List 3 essential criteria for choosing a good shard key (High cardinality, high frequency query targeting, even write distribution).

**Expected output:**
```text
High cardinality, high frequency query targeting, even write distribution
```
...
> ```
>
> **Explanation:** Optimal shard keys distribute writes uniformly and localize read queries to target shards.

### Exercise 3: Sharding Collection with Compound Key

**Problem:** Shard `orders` collection using compound key `{ tenantId: 1, orderId: 1 }`.
