...
## 2. Term Category
- **PostgreSQL Data Type / Column Definition**
...
### Exercise 1: Standard Table Setup

**Problem:** You are creating a table named `logs`. It will hold millions of records, so it needs a large identity column to prevent overflow. Create the table with:
1.  An auto-generated modern identity ID column using the largest integer size.
2.  A text description column `log_msg`.
...
> );
> ```
> - Choose `BIGINT` to support large row counts.
> - Apply the `GENERATED ALWAYS AS IDENTITY` clause.

---



### Exercise 2: Defining Modern Identity Column

**Problem:** Define table `products` using `GENERATED BY DEFAULT AS IDENTITY`.

**Expected output:**
> [!check]- Answer
> ```text
> CREATE TABLE products ( id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT );
> ```
> ```sql
...
> ```
>
> **Explanation:** `GENERATED BY DEFAULT AS IDENTITY` allows manual primary key insertion when required.

---

### Exercise 3: Restarting Identity Sequence Value

**Problem:** Restart sequence for `products.id` identity column to 1000.
...
> ```
>
> **Explanation:** `ALTER COLUMN ... RESTART WITH` resets underlying sequence generators.
