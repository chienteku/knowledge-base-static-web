...
> The massive, automatically generated folder where NPM physically saves the third-party JavaScript code you download from the internet.

---

## 1. Prerequisites
- [NPM (Node Package Manager)](npm.md) — The tool that creates this folder.
- [package.json](package_json.md) — The blueprint that tells NPM what to put inside this folder.
...
**Problem:** You review a Junior Developer's Pull Request. They edited a file located at `node_modules/express/lib/response.js` because they wanted to change how the framework handles cookies. What do you tell them?

**Expected output:**
> [!check]- Answer
> ```text
> "Never edit files inside node_modules!"
> Because node_modules is disposable, the next time someone runs `npm install`, NPM will completely overwrite the folder and delete their custom changes! If you need to change how a framework behaves, you must do it in your own source code (via configuration or monkey-patching), not by hacking the downloaded library files.
> ```
> - What happens to `node_modules` when you type `npm install`?

---



### Exercise 2: Resolving Phantom Dependencies in Flat node_modules

**Problem:** What is a 'phantom dependency' in npm flat `node_modules` structure?

**Expected output:**
> [!check]- Answer
> ```text
> Importing a package that is not declared in package.json, but exists in node_modules because a sub-dependency installed it.
> ```
> ```text
> Importing a package that is not declared in package.json, but exists in node_modules because a sub-dependency installed it.
> ```
>
> **Explanation:** Phantom dependencies break when parent packages update or remove underlying sub-dependencies.

---

### Exercise 3: Cleaning node_modules Directory

**Problem:** Write command to force-delete `node_modules` and re-install clean dependencies.
...
> ```
>
> **Explanation:** Removing `node_modules` ensures a clean dependency graph re-installation.

## 7. Related Terms
- [Module Resolution](../level_03/module_resolution.md) — The exact algorithm Node uses to search inside the `node_modules` folder.
...
## 8. Key Takeaways
- **`node_modules`** is the physical folder where third-party code is downloaded and stored.
- It is massive due to Transitive Dependencies (packages relying on other packages).
- It is 100% disposable. You can delete it and rebuild it at any time using `npm install`.
- ALWAYS add `node_modules` to your `.gitignore` file. Never manually edit files inside of it.
