...
### Exercise 4: Network Packet Alignment & Header Checksum with `#[repr(C)]`

**Problem:**
High-performance network drivers and hardware controllers pass binary packet headers across FFI boundaries. To ensure byte layout alignment matches foreign C structures exactly, structs must be tagged with `#[repr(C)]`.

Define a C-compatible network header structure:
```rust
#[repr(C)]
pub struct NetworkHeader {
    pub version: u8,      // 1 byte (Offset 0)
    pub flags: u8,        // 1 byte (Offset 1)
    pub stream_id: u16,   // 2 bytes (Offset 2)
    pub payload_len: u32, // 4 bytes (Offset 4)
    pub checksum: u32,    // 4 bytes (Offset 8)
}
```

Implement the following:
1. `pub fn verify_header_layout() -> bool`: Uses `std::mem::offset_of!` and `std::mem::size_of!` to verify field offset positions and guarantee total struct size is exactly 12 bytes.
2. `pub extern "C" fn compute_packet_checksum(hdr_ptr: *const NetworkHeader) -> u32`: Calculates expected checksum as `(version + flags + stream_id + payload_len)`. Returns `0` if `hdr_ptr` is null.
3. `pub extern "C" fn validate_and_update_checksum(hdr_ptr: *mut NetworkHeader) -> i32`: Reads the header, computes expected checksum, and updates `(*hdr_ptr).checksum` in-place. Returns `0` on success, or `-1` if `hdr_ptr` is null.

Write unit tests verifying layout offset accuracy, struct size, checksum calculation, and in-place field updates.
...
> use std::ptr;
> 
> /// Network packet header layout matching standard C alignment rules.
> #[repr(C)]
> pub struct NetworkHeader {
>     pub version: u8,
>     pub flags: u8,
...
> }
> 
> /// Verifies that the memory offsets and total size of `NetworkHeader` match C ABI rules.
> pub fn verify_header_layout() -> bool {
>     let off_version = offset_of!(NetworkHeader, version);
>     let off_flags = offset_of!(NetworkHeader, flags);
>     let off_stream = offset_of!(NetworkHeader, stream_id);
>     let off_len = offset_of!(NetworkHeader, payload_len);
>     let off_crc = offset_of!(NetworkHeader, checksum);
>     let total_size = size_of::<NetworkHeader>();
> 
>     off_version == 0
>         && off_flags == 1
>         && off_stream == 2
...
> }
> 
> /// Computes the expected checksum for a network packet header.
> #[no_mangle]
> pub extern "C" fn compute_packet_checksum(hdr_ptr: *const NetworkHeader) -> u32 {
>     if hdr_ptr.is_null() {
>         return 0;
>     }
>     let hdr = unsafe { &*hdr_ptr };
>     (hdr.version as u32) + (hdr.flags as u32) + (hdr.stream_id as u32) + hdr.payload_len
> }
> 
> /// Validates packet header data and updates checksum field in-place.
> #[no_mangle]
> pub extern "C" fn validate_and_update_checksum(hdr_ptr: *mut NetworkHeader) -> i32 {
>     if hdr_ptr.is_null() {
>         return -1;
>     }
>     unsafe {
>         let expected_crc = compute_packet_checksum(hdr_ptr);
>         (*hdr_ptr).checksum = expected_crc;
>     }
>     0
> }
> 
...
> 
>     #[test]
>     fn test_network_header_layout_offsets() {
>         assert!(verify_header_layout());
>         assert_eq!(size_of::<NetworkHeader>(), 12);
>         assert_eq!(offset_of!(NetworkHeader, version), 0);
>         assert_eq!(offset_of!(NetworkHeader, flags), 1);
>         assert_eq!(offset_of!(NetworkHeader, stream_id), 2);
>         assert_eq!(offset_of!(NetworkHeader, payload_len), 4);
>         assert_eq!(offset_of!(NetworkHeader, checksum), 8);
>     }
> 
>     #[test]
>     fn test_checksum_computation_and_update() {
>         let mut packet = NetworkHeader {
>             version: 1,
>             flags: 0x02,
>             stream_id: 10,
>             payload_len: 500,
...
>         };
> 
>         let initial_crc = compute_packet_checksum(&packet);
>         assert_eq!(initial_crc, 513);
> 
>         let status = validate_and_update_checksum(&mut packet);
>         assert_eq!(status, 0);
>         assert_eq!(packet.checksum, 513);
>     }
> 
>     #[test]
>     fn test_null_packet_pointer_safety() {
>         assert_eq!(compute_packet_checksum(ptr::null()), 0);
>         assert_eq!(validate_and_update_checksum(ptr::null_mut()), -1);
>     }
> }
> ```
>
> **Explanation:**
> 1. **`#[repr(C)]` Memory Layout**: By default, `rustc` reorders struct fields for size optimization. `#[repr(C)]` forces struct layout to match C ABI rules, guaranteeing deterministic field ordering and alignment.
> 2. **Layout Offset Verification (`offset_of!`)**: `std::mem::offset_of!(Struct, field)` computes exact field byte offsets, proving compliance with external C ABI specifications.
> 3. **In-Place Field Mutation (`(*hdr_ptr).checksum = ...`)**: Dereferencing a raw mutable struct pointer in an `unsafe` block allows modifying struct memory directly across FFI boundaries.
> 
---
